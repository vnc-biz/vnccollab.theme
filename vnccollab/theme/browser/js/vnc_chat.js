...
vncchat.VncChatRoomView = vncchat.VncChatBoxView.extend({
    ...
    initialize: function () {
        ...
        // TODO: handle error on JOIN and not open room tab, instead provider
        // warning alert to user about error reason
        // TODO: do not escape room names twice in rooms autosuggestion dropdown
...
vncchat.VncRoomsPanel = vncchat.RoomsPanel.extend({
    ...
    updateRoomsList: function () {
        ...
        var rooms = $.grep(tabs, function(tab, i) {
                    return (vncchat.chatboxesview.isChatRoom(tab))});
        if (rooms.length > 0) {
        ...
        for (var i=0; i<rooms.length; i++) {
            var name = Strophe.getNodeFromJid(rooms[i]);
            var open_rooms = jq.map(jq('.chatroom a.open-room'), function(el, i) {
                return jq(el).attr('room-jid')
            });
            if (jq.inArray(rooms[i], open_rooms) == -1) {
                this.$el.find('#available-chatrooms')
                .append(this.room_template({'name':name,
                                            'jid':rooms[i]}));
            }
        }
    },
    createChatRoom: function (ev) {
        ...
        if (ev.type === 'click') {
            ...
            // FIXME: Hardcoded
            var ninput = $(ev.target).find('input.new-chatroom-name');
            name = ninput.val().replace(/^\s+|\s+$/g, '');
            ninput.val('');
                                    ...
                                    .attr('data-recipient', Strophe.escapeNode(obj.id)+'@'+
                                        ...
                                        .text(obj.fullname).on(
                                            'click', function (ev) {
                                                ...
                                                // reset input value
                                                $(ev.target).parents('.inviteParticipant').find('.searchParticipant input.username').val('');
...
vncchat.VncContactsPanel = vncchat.ContactsPanel.extend({
    ...
    subscribeToContact: function (ev) {
        ...
        var jid = $(ev.target).attr('data-recipient');
        //confirm subscription if user already
        //send subscription request to us
        subscriptions = $.grep(vncchat.roster.models, function(e, i) {
                               return (e.get('id') == jid &&
                                       e.get('ask') == 'request')});
        if (subscriptions.length > 0) {
...
vncchat.VncControlBoxView = vncchat.ControlBoxView.extend({
    ...
    initialize: function () {
        ...
        this.on('context-changed', function(ev) {
            var tabs = vncchat.chatboxesview.tabs;
            if (tabs.length > 0) {
                var active_tab = tabs[tabs.length - 1];
                if (vncchat.chatboxesview.isChatRoom(active_tab)) {
                    if (this.$el.hasClass('inChat')) {
                        this.$el.removeClass('inChat');
                    }
                    this.$el.addClass('inRoom');
                } else {
                    if (this.$el.hasClass('inRoom')) {
                        this.$el.removeClass('inRoom');
                    }
                    this.$el.addClass('inChat');
                }

             } else {
                 this.$el.removeClass('inChat inRoom');
             }
        });
